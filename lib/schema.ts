...
export const stageSelectionSchema = z.object({
  ...
  stageReached: z.enum(
    ["depot", "enquete", "etat-lieux", "affichage", "bornage", "evaluation", "decision"],
    {
...
const userProfileSchema = z.object({
  ...
  regularizationCity: z.string().min(1, "La ville de régularisation est requise"),
...
export const surveyFormSchema = z.object({
  ...
  if (!data.dossierId || !data.depositCity || !data.regularizationCity || 
      !data.residenceCity || !data.userType || !data.legalEntity || !data.nationality) {
    ctx.addIssue({
  ...
  if (stageIndex >= 0 && data.depotPaymentMode === "autre" && !data.depotOtherPaymentMode) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (stageIndex >= 1 && data.enquetePaymentMode === "autre" && !data.enqueteOtherPaymentMode) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (stageIndex >= 2 && data.etatLieuxPaymentMode === "autre" && !data.etatLieuxOtherPaymentMode) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (stageIndex >= 4 && data.bornagePaymentMode === "autre" && !data.bornageOtherPaymentMode) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (stageIndex >= 5 && data.evaluationPaymentMode === "autre" && !data.evaluationOtherPaymentMode) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (stageIndex >= 6 && data.decisionPaymentMode === "autre" && !data.decisionOtherPaymentMode) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
  ...
  if (stageIndex >= 3 && data.affichageWasInformed === true && !data.affichageInformationChannel) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
