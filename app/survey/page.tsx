...
export default function SurveyPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(result.error || "Échec de la soumission de l'enquête");
      }
  ...
  const renderStepContent = () => {
    ...
    if (currentStepKey === "profile") {
      return (
        <UserProfileStep formData={formData} updateFormData={updateFormData} />
      );
    }

    if (currentStepKey === "decision") {
      return (
        <DecisionStep formData={formData} updateFormData={updateFormData} />
      );
    }
    ...
    if (currentStepKey === "disputes") {
      return (
        <DisputesStep formData={formData} updateFormData={updateFormData} />
      );
    }
