...
export function DisputesStep({ formData, updateFormData }: DisputesStepProps) {
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <RadioGroup value={formData.hadOpposition} onValueChange={(value) => updateFormData({ hadOpposition: value })}>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="oui" id="had-opposition-yes" />
            <Label htmlFor="had-opposition-yes" className="font-normal cursor-pointer">Oui</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="non" id="had-opposition-no" />
      ...
      <div className="space-y-2">
        <Label htmlFor="oppositionDate">À quelle date l'opposition a-t-elle été enregistrée ?{formData.hadOpposition === "oui" && "*"}</Label>
        <Input
          id="oppositionDate"
          type="date"
          value={formData.oppositionDate}
          onChange={(e) => updateFormData({ oppositionDate: e.target.value })}
          disabled={formData.hadOpposition !== "oui"}
        />
      </div>

      <div className="space-y-2">
        <Label>Quelle est la nature de l'opposition ?{formData.hadOpposition === "oui" && "*"}</Label>
        <Select 
      ...
      {formData.oppositionNature === "autre" && formData.hadOpposition === "oui" && (
        <div className="space-y-2">
      ...
      <div className="space-y-2">
        <Label htmlFor="litigeDelay">Délais de traitement du litige (dépôt → décision) en jours{formData.hadOpposition === "oui" && "*"}</Label>
        <Input
          id="litigeDelay"
          type="number"
      ...
      <div className="space-y-2">
        <Label>Avez-vous payé des frais liés à l'opposition/contentieux ?{formData.hadOpposition === "oui" && "*"}</Label>
        <RadioGroup 
          value={formData.paidLitigeFees} 
          onValueChange={(value) => updateFormData({ paidLitigeFees: value })}
          disabled={formData.hadOpposition !== "oui"}
        >
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="oui" id="paid-fees-yes" disabled={formData.hadOpposition !== "oui"} />
            <Label htmlFor="paid-fees-yes" className="font-normal cursor-pointer">Oui</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="non" id="paid-fees-no" disabled={formData.hadOpposition !== "oui"} />
      ...
      <div className="space-y-2">
        <Label>Mode de paiement{formData.hadOpposition === "oui" && formData.paidLitigeFees === "oui" && "*"}</Label>
        <Select 
          value={formData.litigePaymentMode} 
          onValueChange={(value) => updateFormData({ litigePaymentMode: value })}
          disabled={formData.hadOpposition !== "oui" || formData.paidLitigeFees !== "oui"}
        >
          <SelectTrigger>
      ...
      <div className="space-y-2">
        <Label htmlFor="litigePaymentAmount">Montant{formData.hadOpposition === "oui" && formData.paidLitigeFees === "oui" && "*"}</Label>
        <Input
      ...
      <div className="space-y-2">
        <Label>Avez-vous reçu une quittance pour ces frais ?{formData.hadOpposition === "oui" && formData.paidLitigeFees === "oui" && "*"}</Label>
        <RadioGroup 
          value={formData.litigeHasReceipt} 
          onValueChange={(value) => updateFormData({ litigeHasReceipt: value })}
          disabled={formData.hadOpposition !== "oui" || formData.paidLitigeFees !== "oui"}
        >
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="oui" id="litige-receipt-yes" disabled={formData.hadOpposition !== "oui" || formData.paidLitigeFees !== "oui"} />
            <Label htmlFor="litige-receipt-yes" className="font-normal cursor-pointer">Oui</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="non" id="litige-receipt-no" disabled={formData.hadOpposition !== "oui" || formData.paidLitigeFees !== "oui"} />
      ...
      <div className="space-y-2">
        <Label>Avez-vous été informé de la procédure et des voies de recours ?{formData.hadOpposition === "oui" && "*"}</Label>
        <RadioGroup 
          value={formData.wasInformedProcedure} 
          onValueChange={(value) => updateFormData({ wasInformedProcedure: value })}
          disabled={formData.hadOpposition !== "oui"}
        >
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="oui" id="informed-procedure-yes" disabled={formData.hadOpposition !== "oui"} />
            <Label htmlFor="informed-procedure-yes" className="font-normal cursor-pointer">Oui</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="non" id="informed-procedure-no" disabled={formData.hadOpposition !== "oui"} />
      ...
      <div className="space-y-2">
        <Label>Avez-vous adressé un courrier formel (ex. opposition au DG ANUTTC) ?{formData.hadOpposition === "oui" && "*"}</Label>
        <RadioGroup 
          value={formData.sentFormalLetter} 
          onValueChange={(value) => updateFormData({ sentFormalLetter: value })}
          disabled={formData.hadOpposition !== "oui"}
        >
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="oui" id="letter-yes" disabled={formData.hadOpposition !== "oui"} />
            <Label htmlFor="letter-yes" className="font-normal cursor-pointer">Oui</Label>
          </div>
          <div className="flex items-center space-x-2">
            <RadioGroupItem value="non" id="letter-no" disabled={formData.hadOpposition !== "oui"} />
            <Label htmlFor="letter-no" className="font-normal cursor-pointer">Non</Label>
          </div>
        </RadioGroup>
      </div>

      {formData.sentFormalLetter === "oui" && formData.hadOpposition === "oui" && (
        <div className="space-y-2">
      ...
      <div className="space-y-2">
        <Label>Cause perçue du litige{formData.hadOpposition === "oui" && "*"}</Label>
        <Select 
      ...
      {formData.litigeCause === "autre" && formData.hadOpposition === "oui" && (
        <div className="space-y-2">
      ...
      <div className="space-y-2">
        <Label>Satisfaction sur la gestion du litige{formData.hadOpposition === "oui" && "*"}</Label>
        <StarRating
          value={formData.litigeSatisfaction[0] || 0}
          onChange={(value) => updateFormData({ litigeSatisfaction: [value] })}
          max={5}
          size="lg"
          readonly={formData.hadOpposition !== "oui"}
        />
      </div>

      <div className="space-y-2">
        <Label>Issue du litige{formData.hadOpposition === "oui" && "*"}</Label>
        <Select 
      ...
      {formData.litigeOutcome === "autre" && formData.hadOpposition === "oui" && (
        <div className="space-y-2">
...
export function validateDisputesStep(
  ...
  if (!formData.hadOpposition) return false;
  
  if (formData.hadOpposition === "non") return true;
  
  const baseValidation = !!(
    formData.oppositionDate &&
    formData.oppositionNature &&
    formData.litigeDelay &&
    formData.paidLitigeFees &&
    formData.wasInformedProcedure &&
    formData.sentFormalLetter &&
  ...
  if (formData.sentFormalLetter === "oui" && !formData.letterReference) return false;
  
  if (formData.paidLitigeFees === "oui") {
    return !!(
      formData.litigePaymentMode &&
      formData.litigePaymentAmount &&
      formData.litigeHasReceipt
    );
  }
